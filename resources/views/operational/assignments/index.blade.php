...
@section('content')
    ...
    <div class="bg-gray-100 flex-1 p-6 md:mt-16">
        ...
        <div class="flex mb-4 border-b border-gray-300">
            <button class="px-4 py-2 text-gray-700 font-bold border-b-2 border-indigo-500 focus:outline-none" id="btn-pool"
                onclick="showTab('pool')">
                Open Pool ({{ $openTickets->count() }})
            </button>
            <button class="px-4 py-2 text-gray-500 font-bold hover:text-gray-700 focus:outline-none" id="btn-my"
                onclick="showTab('my')">
            ...
            <div class="card bg-white border rounded shadow-md w-full">
                ...
                <div class="card-body p-0 overflow-x-auto">
                    <table class="w-full text-left border-collapse table-auto">
                        <thead>
                            <tr
                                class="bg-gray-100 text-gray-700 text-xs uppercase tracking-wider font-bold border-b border-gray-200">
                        ...
                        <tbody class="text-gray-600 text-sm font-light">
                                ...
                                <tr class="border-b border-gray-200 hover:bg-gray-100 transition duration-300 ease-in-out">
                                    <td class="px-6 py-4">
                                        <div class="flex items-center">
                                            <div
                                                class="w-8 h-8 rounded-full bg-indigo-100 flex items-center justify-center mr-3 text-indigo-700 font-bold text-xs">
                                                {{ substr($ticket->customer->instance ?? $ticket->customer->customer_name, 0, 1) }}
                                            </div>
                                            <div>
                                                <span
                                                    class="font-bold text-gray-800 block">{{ $ticket->customer->instance ?? 'Unknown' }}</span>
                                                <span
                                                    class="text-xs text-gray-500">{{ $ticket->customer->customer_name ?? '-' }}</span>
                                            </div>
                                        </div>
                                    </td>
                                    <td class="px-6 py-4">
                                    ...
                                    <td class="px-6 py-4 text-center">
                                        ...
                                        @endphp
                                        <span
                                            class="bg-{{ $color }}-200 text-{{ $color }}-700 py-1 px-3 rounded-full text-xs font-bold">
                                    ...
                                    <td class="px-6 py-4 text-center">
                                        <div class="flex items-center justify-center space-x-3">
                                            ...
                                            <a href="{{ route('assignments.show', $ticket->visit_ticket_id) }}"
                                                class="w-4 text-gray-500 hover:text-indigo-700 transform hover:scale-110 transition-transform"
                                                title="Lihat Detail">
                                                <i class="fad fa-eye fa-lg"></i>
                                            </a>

                                            <button onclick="takeJob('{{ $ticket->visit_ticket_id }}', this)"
                                                class="bg-green-500 hover:bg-green-600 text-gray-500 py-1 px-3 rounded text-xs font-bold shadow transition-all flex items-center">
                            ...
                            @empty
                                <tr>
                                    <td colspan="5" class="px-6 py-4 text-center text-gray-500">Tidak ada pekerjaan
                                        tersedia.</td>
                                </tr>
                            @endforelse
                        </tbody>
                        ...
                        <tbody class="text-gray-600 text-sm font-light">
                            @forelse($myAssignments as $assignment)
                                <tr class="border-b border-gray-200 hover:bg-gray-100 transition duration-300 ease-in-out">
                                    ...
                                    <td class="px-6 py-4">
                                        <span
                                            class="bg-indigo-200 text-indigo-700 py-1 px-3 rounded-full text-xs font-bold">
                                            {{ $assignment->status }}
                                        </span>
                                    </td>
                                    <td class="px-6 py-4 text-center">
                                        <a href="{{ route('assignments.show', $assignment->visit_ticket_id) }}"
                                            class="bg-gray-600 hover:bg-gray-700 text-white py-1 px-3 rounded text-xs font-bold shadow transition-all">
                            ...
                            @empty
                                <tr>
                                    <td colspan="4" class="px-6 py-4 text-center text-gray-500">Anda belum mengambil
                                        pekerjaan.</td>
                                </tr>
                            @endforelse
                        </tbody>
